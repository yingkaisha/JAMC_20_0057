...
for sea in seasons:
    ...
    # XNET configuration
    dscale_xnet = mu.XNET(N, (None, None, N_input), pool=pool, activation=activation)
    opt_ = keras.optimizers.Adam(lr=lr)
    dscale_xnet.compile(loss=keras.losses.mean_absolute_error, optimizer=opt_)
    
    # check point settings
    # "temp_dir" is where models are saved, defined in the namelist.py
    save_name = 'XNET_raw_{}_{}'.format(VAR, sea)
    ...
    for i in range(epochs):
        ...
        if i == 0:
            Y_pred = dscale_xnet.predict([X_valid])
        ...
        for j, name in enumerate(trainfiles): 
            ...
            # train on batch
            loss_ = dscale_xnet.train_on_batch(X, Y)
